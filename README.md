...
- 🧯 **预算熔断** — 轮数与 Token 双预算，超限自动「闭麦」，可补预算继续或汇总收场；**任一轴填 0 = 不限制**（该轴永不闭麦，界面显示 `∞`）
...
选择「多模型平等」时界面会要求填安全限制（最大轮数、最大 Token），超限即闭麦暂停；两轴都填 0 即完全不限制（此时会议只能靠主持人判断收场）。选择「针锋相对」时总纲自动附加红队评审协议（只挑毛病、不给替代方案）。
...
| 最大轮数 / 最大 Token | 10 / 200000 | 会议预算，超限闭麦；**填 0 = 不限制**（该轴永不闭麦） |
| 互通开关 | 开 | 只看当前对话会议 / 看全部 |
...
- **「圆桌会议」Tab** — 打开该 Tab 等于开启讨论模式（切走即关），另含**两个视图**（顶部切换条）：
  ...
  - **群聊**（v0.2.47）— QQ 群聊形态的发言流（气泡 / 品牌头像 / 连续同人合并 / 日期与轮次分隔）+ 底部输入框（Enter 发送、Shift+Enter 换行），可**以主持人身份直接往会议里说一句**；说出的这句会**唤醒主持人**（落盘 ≠ 送达，没能送达时界面如实提示）
...
> 输入框发出的消息落成 `nodeKey=captain` + `source=user` 的一条发言，**不计入会议预算**（用户发言不消耗专家轮次与 token）；该标记会随 status / 汇聚网关 / 导出一起送到主持人（v0.2.49），使它能分辨"用户亲口说的"与"我自己说的"。
> 输入框的 Enter 发送**带 IME 组合态守卫**（v0.2.49）：中文/日文输入法选词时按 Enter 不会误发半截内容。
