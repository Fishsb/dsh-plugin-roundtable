...
/*
 * ## O1 边界登记（2026-09-26 · maint 席复算订正；位置 = 本文件末尾）
 *
 * 现象：接线判据若收到**未去注释的裸 raw**，stripStrings 在这种输入上**不保行号** ——
 *   本文件里那两道**独立成行的三反引号围栏**被当成跨行模板，吞掉 4 个换行。
 *   （⚠ 它们**不在文件头**，而在文件靠后的位置 —— 故本段只说「第几道围栏」、不说「文件头」，
 *     免得读者去文件开头找一个离那里上千行的东西。）
 *
 * ⚠ 本段刻意**只写不变式、不写绝对行号**（行号会随任何编辑漂移，正是 O1 本身要治的病）：
 *   · **不变式 1**：吞行数恒为 **4**；
 *   · **不变式 2**：**第一处错位起点 = 第一道三反引号围栏所在的那一行**（不是「其后第 N 行」）；
 *   · **不变式 3**：本门按行号消费的针**全部位于错位起点之前** —— 与「当前无后果」互为印证。
 *
 * 订正来源：上一轮登记用的是**绝对行号**，且与实测**差 3 行**（它写成错位点之后第 3 行，
 *   实测为第一道围栏所在行本身）。本文件此后每次增删都会让任何绝对行号再漂，
 *   故此处改按上面三条不变式登记；若需具体行号，按下面命令**现算**，不要抄旧值。
 *
 * 为什么当前无后果：错位不落在任何被消费的行上（见不变式 3）。
 *   方向亦只在**误红**侧：针不会因错位而消失，故不可能静默假绿。
 *
 * 处置：**保持有界，不消除**（与 k1 的停止条件同口径）。本登记**不新增任何断言**、不改任何判据逻辑。
 * 复验（只读、不改盘；脚本属 .audit/ scratch，被清理后按上面三条不变式自行复算）：
 *   node .audit/verify/o1-probe3.mjs
 */
