  ...
  return [
    ...
    ...(meeting.boundary === undefined
      ...
      : [
          ...
          `- 怎么检查才算达标：${meeting.boundary.check.trim() === '' ? '（未声明）' : meeting.boundary.check.trim()}`,
    ...
    /*
     * 执行协议（P3，2026-09-25 · 用户要求「团队角色不仅审查方案还要做方案落地执行」）。
     *
     * 为什么**条件拼接**（仅 boundary 非空时出现）：执行段的全部内容都建立在
     * 「有一面不许动的墙」之上（写前声明、写后交证据、越界即停）。没有边界声明的
     * 会议（例如纯勘察/脚本探针会议）挂上这段只会得到无判据可依的授权文案，
     * 反而把"未声明边界即允许写"这个最危险的默认值写进 persona。
     *
     * 与 P6 的关系：工具面（allow 恒下发）决定专家**能**写；本段决定专家**获准**
     * 写、以及写完怎么交代。两者必须同批落地，否则就是"机制说可以、文案说本职
     * 不许"的双真相（本仓 `test/double-truth-guards.test.mjs` 已在剿这类形态）。
     */
    ...(meeting.boundary === undefined
      || meeting.boundary.notDoing.trim() === ''
      ? []
      : [
          '',
          '一之三、执行协议（**你获准动手改工作区**，但须按此交代）',
          '- 你的角色**不限于审查**：kind 为 `change` 的工作项，你就是执行者，请直接修改工作区文件并让改动真的生效（改源码 ≠ 生效：本插件产物在 `lib/`，宿主加载的是 profile 装载副本）。',
          '- 执行前：在 [核心产出] 里**先说清要碰哪些文件**，并说明这一步可能碰坏哪面墙。',
          '- 执行后：在 [核心产出] 里**列出你实际改动的文件路径**，并给出「原行为 + 凭什么确认它仍成立」的证据；给不出证据就标「未验证」。',
          '- 改完必须重建并同步产物，否则宿主仍加载旧代 —— 只改源码而不重建，一律视为**未生效**，不得声明完成。',
          '- 同树并发：**同一波次内不要与他人改同一文件**；需要改同一文件时，用 `depends_on` 把它排到后一波（写串行，读可并行）。**写-读也按此办**：本波正在被**读**的文件（复核、勘察、只读取证）同样不得改 —— 改之前先从计划里认出这些读席，认不出就当场问主持人，别默认"没人说就是在没读"；确实必须改，则改完**在 [核心产出] 里报出改动的文件与时刻**，那是读席唯一能发现"我读到的是中间态"的通道（写与读同时发生时，读到的字节既不属于写前、也不属于写后任何一代）。',
          '- 越界即停：若某处改动确需触碰上面「不许动」的墙，**先停下并说明**，交主持人或用户拍板，不得自行放宽边界。',
        ]),
    '',
    ...
    '- 超预算（轮数/Token）时会议将自动闭麦暂停；某条轴的预算为 0 表示该轴不限制（永不闭麦）。',
    ...redteamRules,
  ].join('\n')
}
