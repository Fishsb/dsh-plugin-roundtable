 ...
 * 导出整席丢失，且全程**不报错**。本场现场**至少复发 3 次**（`minimal` R1、`user` R2、
 * `impl` R1–R2 连续两轮；第 4 轮 `user` 与 `impl` 再次同型，`user` 两次要求后仍未入账）。
 *
 * ⚠ **本判据对"补录"是盲的**：补录按**原轮次**入账（这是有意的，否则晚到的发言会
 * 污染后续轮的分母），于是机器口径下本场读数是 `emptyRounds=0 / silentSeats=0 /
 * lateReplies=1`（`impl` R1→R3），而人工看到的复发远不止 3 次。**判据只测"此刻账本是否
 * 完整"，不测"过程中是否曾经缺账"** —— 后者靠主持人当场补录与代录，机器抓不到。
 * 跨 19 场历史基线为 empty-round 1/25（可为 0）。
 *
